...
flowchart TD
    ...
    subgraph HYB [Гибридный поиск: два независимых прохода по одному индексу]
        direction LR
        DS[по векторам<br/>ловит смысл<br/>top-40]:::code
        BM[BM25 по словам<br/>ловит термины и числа<br/>top-40]:::code
    end

    RW --> DS
    RW --> BM
    KB[(LanceDB<br/>1604 статьи PubMed<br/>4617 чанков)]:::store -.-> DS
    KB -.-> BM

    DS --> RRF[RRF: слияние по позициям<br/>80 кандидатов → 5]:::code
    BM --> RRF
    ...
    G -->|нет, попробуем ещё| RFN[refine<br/>переформулировать запрос]:::llm
    RFN --> DS
    RFN --> BM
...
На схеме между ними нет стрелки, и это не упущение. Они не обмениваются
ничем: один считает косинусную близость эмбеддингов, другой — частоты
слов, и ни один не читает выход другого. Независимость здесь не побочное
свойство, а условие работы: **Reciprocal Rank Fusion складывает два
ранжирования как два независимых свидетельства**. Выводись одно из
другого — сливать было бы нечего.

RRF сравнивает только **позиции**, поэтому косинусную близость
и BM25-score не нужно приводить к общей шкале: у них разная природа,
и любая попытка их взвесить добавила бы ещё один подбираемый параметр.
Каждый поиск отдаёт по 40 кандидатов, после слияния остаётся 5.
